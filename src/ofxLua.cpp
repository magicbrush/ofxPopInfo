 ...
 *
 * Lua, Copyright (c) 1994?011 Lua.org, PUC-Rio using the MIT License.
...
//------------------------------------------------------------------------------
void ofxLua::scriptFcn( string FcnName )
{
  if(L == NULL || !isFunction(FcnName))
    return;
  lua_getglobal(L, FcnName.c_str());
  if(lua_pcall(L, 0, 0, 0) != 0) {
    string msg = "Error running "+ FcnName + " " + (string) lua_tostring(L, LUA_STACK_TOP);
    errorOccurred(msg);
  }
}
...
bool ofxLua::isInited()
{
  return bInited;
}

void ofxLua::clearAllofxLuas()
{
  for(auto L:pLuas)L->clear();
}

set<ofxLua*> ofxLua::pLuas;
